...
```{r, include=FALSE }
# ls_stack <- raster::stack(landsat_file) # read all 3 bands as raster stack
# plot(ls_stack) 
# plotRGB(ls_stack, scale=1.5)  # scale = 1.5 bc bands go from 0 to 1
```
...
# --- plot RGB bands
color_graph(red, RGB[1])
color_graph(green, RGB[2])
color_graph(blue, RGB[3])
...
mask(green, mex_rast) %>% plot(col = hcl.colors(n = 100, palette = 'Greens 2'))
mask(blue, mex_rast) %>% plot(col = hcl.colors(n = 100, palette = 'Blues 2'))
